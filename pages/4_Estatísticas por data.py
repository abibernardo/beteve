...
st.title("Estatísticas por período de tempo")
st.divider()
col1, col2, col3 = st.columns(3)
with col1:
    input = st.text_input("Jogador", placeholder="James Harden")
with col2:
    inicio = st.date_input("Desde", datetime.date(mes_passado))
with col3:
    fim = st.date_input("Até", datetime.date(hoje))
    if input:
        try:
...
if input:
    ...
    if 'nome' in locals():
        ...
        try:
            df_jogos = df_jogos.with_columns(pl.col("GAME_DATE").str.strptime(pl.Date, "%Y-%m-%d"))
            df_jogos = df_jogos.filter(pl.col("GAME_DATE").is_between(inicio, fim))
            df_jogos = (
                df_jogos
            ...
            njog = df_jogos.height
            st.write(f"## {njog} jogos de {nome} no período")
            cols = st.columns(5)
            ...
            st.write(f"## {nome} não teve jogos nesse período.")
        ...
        if df_jogos.height > 1:
            with col11:
                stat = st.selectbox(
                    'Estatística de interesse',
                    stats, key='stats')
            st.write(f"## {stat} de {nome}")
            st.write(f"{inicio} até {fim}")
